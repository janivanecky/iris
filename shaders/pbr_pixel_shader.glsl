...
{
	return F0 + (vec4(1,1,1,1) - F0) * pow(1 - dot(l,h),5); 
}
...
void main()
{
	...
	vec3 lightPos = vec3(00.0f,50.0f,20.0f);//float3(4,4,4);
	lightPos = vec3(0,50,00.0);
	vec3 lightDir = normalize(lightPos - worldPos);
	//lightDir = normalize(vec3(2,1,3));
	//lightDir = normalize(lightPos);
	vec3 normal_ = normalize(normal.xyz);
	lightDir = (view_matrix * vec4(0, 1, 0, 0.0)).xyz;
	...
	vec4 col = lightNormalDot * lightColor * PI *
		BRDF(normal_, lightDir, camDir, specularColor, diffuseColor, roughness);
